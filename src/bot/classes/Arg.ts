...
import { getRESTUser } from "../utils";
import type { ArgTypes, ParsedArgs } from "typings/args";
    ...
    this.argtypes = {
      ...
      boolean: (a, _msg, flag) => {
        if (
          ...
          a === "true" ||
          a === "enable" ||
          (flag?.includes("strict") ? a === "yes" : a.startsWith("y"))
        )
          return true;
        if (
          a === "off" ||
          a === "false" ||
          a === "disable" ||
          (flag?.includes("strict") ? a === "no" : a.startsWith("n"))
        )
          return false;
        return;
      },
      ...
      channel: (a, msg, flag) => {
        const channel = msg.channel.guild.channels.find(
          (c) =>
            c.id === a || a.startsWith(`<#${c.id}>`) || c.name.startsWith(a)
        );
      ...
      channelArray: (a, msg) => {
        ...
        a.forEach((i: string) => {
          const channel = msg.channel.guild.channels.find(
            (c) =>
              c.name.toLowerCase().startsWith(i.toLowerCase()) ||
              c.id === i ||
              i === `<#${c.id}>`
          );
      ...
      member: (a, msg, flag) => {
        ...
        if (!member && flag?.includes("userFallback"))
          return this.argtypes.user(a, msg, ["REST"]);
        if ((!a || !member) && flag?.includes("fallback"))
          return msg.channel.guild?.members?.get(msg.author.id);
      ...
      role: (a, msg) => {
        const role = msg.channel.guild.roles.find(
          (r) =>
            (r.id === a ||
              a.startsWith(`<@&${r.id}>`) ||
              r.name.toLowerCase().startsWith(a.toLowerCase())) &&
            !r.managed
        );
      ...
      roleArray: (a, msg) => {
        ...
        a.forEach((i) => {
          const role = msg.channel.guild.roles.find(
            (r) =>
              (r.name.toLowerCase().startsWith(i.toLowerCase()) ||
                r.id === i ||
                i === `<@&${r.id}>`) &&
              !r.managed
          );
      ...
      user: async (a, _msg, flag) => {
        ...
        if (!user?.id && flag?.includes("REST"))
          user = await getRESTUser(a, this.bot);
        if (!user?.id) return;
        return user;
      },
    };

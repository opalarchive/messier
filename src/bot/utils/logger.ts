...
const consoleLogFormat = winston.format.combine(
  ...
  winston.format.printf(
    (info) =>
      `${dateLogFormat(info.timestamp)} (${info.level}): ${info.message}`
  )
);
...
const logFormat = winston.format.combine(
  winston.format.timestamp(),
  winston.format.printf(
    (info) => `${dateLogFormat(info.timestamp, false)}: ${info.message}`
  )
);

// Rotates logs daily
const dailyTransport = new winston.transports.DailyRotateFile({
  filename: "messier-%DATE%.log",
  ...
  maxSize: "20m",
  maxFiles: "14d",
  auditFile: `${LOGS_DIRECTORY}/messier-${level}-audit.json`,
});
